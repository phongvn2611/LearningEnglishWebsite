...
function makeRoleSelector(role, exact, customNormalizer) {
  ...
  const roleRelations = _ariaQuery.roleElements.get(role);

  const implicitRoleSelectors = roleRelations && new Set(Array.from(roleRelations).map(({
    name
  }) => name));
  return [explicitRoleSelector, ...(implicitRoleSelectors != null ? implicitRoleSelectors : [])].join(',');
}
