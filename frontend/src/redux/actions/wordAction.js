...
export const getWord = (id) => {
    return async (dispatch, getState) => {
        try { 
          ...
          dispatch({ type: WORD_CONSTANT.SET_WORD_ERROR })
...
export const postWord = (formData) => {
    return async (dispatch, getState) => {
        try { 
          ...
          const response = await wordApi.postWord(formData, user.access_token);
            if(response.status===200){
              dispatch({
...
export const deleteWord = (id) => {
    return async (dispatch, getState) => {
        try { 
          ...
          const response = await wordApi.deleteWord(id, user.access_token);
            if(response.status===200){
              dispatch({
